...
import BotonAccion from "../botones/BotonAccionSolicitarNuevaSolicitud";
...
export default function InventarioTable({
  ...
  // 1. CORREGIDO: Declaración correcta del estado de ítems surtidos
  const [surtidoItems, setSurtidoItems] = useState<(Insumo & { cantidad: number })[]>([]);

  // 2. CORREGIDO: Se agregó el estado faltante para controlar el Drawer
  ...
  const abrirDrawerSurtido = (item: Insumo) => {
    ...
    setDrawerSurtido(true); // Ahora sí existe este estado cambiador
  };
      ...
      <Card withBorder radius="md" shadow="sm" mt="md" style={{ borderLeft: "6px solid #0b6fa4", background: "#f8fbfd" }}>
        ...
        <ScrollArea h={500} viewportRef={viewportRef} onScrollPositionChange={handleScroll}>
          {loading ? (
            ...
            /* 3. MEJORA TS: Añadimos el tipo genérico <Insumo> a la DataTable */
            <DataTable<Insumo>
              withTableBorder
              highlightOnHover
              striped
              ...
              columns={[
                ...
                { accessor: "codigo_barras", title: "Código", render: (r) => <Code fw={700}>{r.codigo_barras || "—"}</Code> },
                {
                  accessor: "accion",
                  title: "Acción",
                  textAlign: "center",
